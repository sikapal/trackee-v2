...
public class PackageService {
    ...
    public Iterable<Package> gePackagesArrived(String destination) {
	...
	public boolean try_packet_id(String matricule){
		Iterable<Package> packets = this.gePackages();
		Iterator<Package> iterator = packets.iterator();
		while(iterator.hasNext()) {
			Package element = iterator.next();
			if(matricule.equals(element.getNumSerie())){
				return true;
			}
		}
		return false;
